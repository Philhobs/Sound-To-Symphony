...
class Autoencoder(Model):
    ...
    def generate_random_sounds(self, n_samples, mean=0, scale=1):
        ...
        for n in range(n_samples):
            ...
            random_sound = librosa.feature.inverse.mel_to_audio(decoded_mel,  n_fft=2500, sr=self.sample_rate)
...
if __name__ == "__main__":
    ...
    autoencoder.fit_training_sound('./audio/AMINOR.wav', epochs=50)

    ''' Create hybrid sound '''
    warped = autoencoder.warp_sample('./audio/SYNTH_sample.wav')
    sf.write(f"./warped_sound_{time.time()}.wav", warped, autoencoder.sample_rate)

    ''' Generate random sounds '''
    #warped_noise_sample = autoencoder.generate_random_sounds(n_samples=3, mean=0, scale=3)

    #for x in range(len(warped_noise_sample)):
        #sf.write(f"./warped_sound_{time.time()}.wav", warped_noise_sample[x], autoencoder.sample_rate)
